...
// ✅ PDPA: ไม่ query Turso, ไม่มี HN/ชื่อ, link หมดอายุ 4 ชม.

export async function onRequestGet(context) {
  const url = new URL(context.request.url);
  const encoded = url.searchParams.get('d');

  let d = null, expired = false, decodeError = false;
...
function sevColor(n) { n=Number(n); return n<=4?'#3B6D11':n<=15?'#854F0B':'#A32D2D'; }
function sevBg(n)    { n=Number(n); return n<=4?'#EAF3DE':n<=15?'#FAEEDA':'#FCEBEB'; }
function sevLabel(n) {
  n=Number(n);
  if(n===0)return'No stroke'; if(n<=4)return'Minor';
  if(n<=15)return'Moderate';  if(n<=20)return'Moderate-Severe'; return'Severe';
}
