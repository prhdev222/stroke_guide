...
**QR สองอัน (หลังสร้างลิงก์):** ลิงก์/QR **ดู** (`/ct-temp/view?t=...`) สำหรับ **Neuro** — เปิดดูภาพได้ทันที **ไม่มี PIN** • **QR ฝั่งพยาบาล** ชี้ไป `GET /ct-temp/mobile-upload?k=<mobileKey>` — **เท่านั้นที่ต้องกรอกรหัส 4 หลัก** จากจอ Staff แล้วได้ **write token** สำหรับอัปโหลด • Staff อัปโหลดจาก wizard ใช้ `uploadToken` (Bearer) ไม่ต้อง PIN • QR สร้างด้วย [qrcodejs](https://www.npmjs.com/package/qrcodejs) จาก jsDelivr
...
**ถ่ายวิดีโอในมือถือ (Staff wizard):** หลังสร้างลิงก์ บล็อกถ่ายคลิปแสดงเมื่อ **จอแคบ (≤768px)** หรือ **อุปกรณ์สัมผัส** (`hover: none` + `pointer: coarse`) เพื่อให้มือถือแนวนอน (เช่น Samsung) ยังเห็นปุ่ม **เริ่มถ่าย / หยุด / สลับกล้อง** — ใช้ `getUserMedia` + `MediaRecorder` (ต้อง HTTPS) • เลือกวิดีโอจากแกลเลอรีใช้ช่อง **วิดีโอ** แยกจากรูป (Android มักเลือกวิดีโอไม่ได้ถ้ารวม `accept` กับรูป); เบราว์เซอร์ที่ไม่รองรับจะซ่อนบล็อกถ่ายคลิป
...
| `POST` | `/ct-temp/upload` | `Authorization: Bearer <uploadToken>` (Staff) **หรือ** `Bearer <writeToken>` (มือถือหลังยืนยัน PIN) — `multipart` ฟิลด์ `file` หลายไฟล์ • session ที่มี `uploadPin` จะไม่รับ Bearer เป็น `uploadToken` จากมือถือยกเว้นผ่าน `writeToken` |
| `GET` | `/ct-temp/view?t=<viewToken>` | หน้า viewer: ซูมภาพ (+/−), วิดีโอปรับความเร็ว (slow motion), ปุ่มบันทึก/ดาวน์โหลด • `&raw=1&i=0` = ไฟล์ดิบ |
| `GET` | `/ct-temp/mobile-upload?k=<mobileKey>` | หน้ามือถือ: กรอกรหัส 4 หลัก → `POST /ct-temp/mobile-verify` → ได้ `writeToken` → อัปโหลด |
| `POST` | `/ct-temp/mobile-verify` | body JSON `{ "k": "<mobileKey>", "pin": "1234" }` (หรือใส่ `"u": "<uploadToken>"` แทน `k` สำหรับลิงก์เก่า) → `{ "writeToken" }` |
| `GET` | `/ct-temp/mobile-upload?u=<uploadToken>` | รองรับ session **เก่า**ที่ไม่มี PIN — หรือ session ใหม่ที่มี PIN จะให้กรอกรหัสในหน้าเดียวกัน |
